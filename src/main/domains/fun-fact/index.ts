...
import { createModelFromSettings, streamText, invokeText } from '@main-core/ai-service'
...
async function requestFromAiStreamed(args: {
  ...
  const { model } = createModelFromSettings(args.settings, {
    temperature: 0.7,
    maxTokens: 220
  })
  const messages = [
    ...
    new HumanMessage(buildUserPrompt(args.settings, args.ymd))
  ]
...
async function requestFromAi(settings: AppSettings, ymd: string): Promise<string> {
  ...
  const messages = [
    ...
    new HumanMessage(buildUserPrompt(settings, ymd))
  ]
